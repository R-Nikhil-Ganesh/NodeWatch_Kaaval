...
export const PoliceDashboard = ({ onNavigate }: DashboardProps) => {
    ...
    const myCases = sortCasesByPriority(
        cases.filter(c => c.createdBy === currentUser?.id || c.status === CaseStatus.OPEN || c.status === CaseStatus.UNDER_INVESTIGATION)
    );
        ...
        <div className="space-y-6">
            ...
            <Card title="Available Cases">
                {myCases.length === 0 ? (
                    ...
                    <Table headers={['Case ID', 'Title', 'Priority', 'Date Created', 'Status', 'Action']}>
                        {myCases.map(c => (
