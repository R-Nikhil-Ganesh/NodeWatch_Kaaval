...
const IntegrityBadge = ({ status }: { status: IntegrityStatus }) => {
  // Evidence rows are written by the Police/Forensics side (frontend_web),
  // whose IntegrityStatus enum also has NOT_CHECKED — a value this narrower
  // Legal-side type doesn't declare. Fall back rather than crash on it.
  const iconMap = { VERIFIED: ShieldCheck, COMPROMISED: ShieldAlert, PENDING: ShieldQuestion } as const;
  const labelMap = { VERIFIED: 'Verified', COMPROMISED: 'Tamper Detected', PENDING: 'Pending Verification' } as const;
  const Icon = iconMap[status as keyof typeof iconMap] ?? ShieldQuestion;
  const label = labelMap[status as keyof typeof labelMap] ?? 'Pending Verification';
  return (
